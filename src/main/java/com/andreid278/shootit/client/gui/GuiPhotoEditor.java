...
public class GuiPhotoEditor extends Gui {
	...
	public void draw(Minecraft mc, int mouseX, int mouseY) {
		mc.renderEngine.bindTexture(Resources.TRANSPARENCY);
		...
		if(photoID != -1) {
			...
			if(!isEnabled) {
				mc.renderEngine.bindTexture(PhotosData.photos.get(photoID));
				drawTexturedModalRect(x, y, 0, 0, width, height, 1, 1);
				return;
			}
	...
	public void setEnabled(boolean isEnabled) {
		...
		if(isEnabled && photoID != -1) {
			try {
				File file = new File(MCData.photosFolderPathClient + "/" + photoID + ".png");
				if(!file.exists()) {
	...
	public void save() {
		...
		imageBuffer.rewind();
		try {
			ImageIO.write(bufferedImage, "png", new File(MCData.photosFolderPathClient + "/" + "0.png"));
		} catch (IOException e) {
