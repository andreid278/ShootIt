...
public class GuiEditor extends Gui {
	...
	public void draw(Minecraft mc, int mouseX, int mouseY) {
		if(photoID == -2)
			mc.renderEngine.bindTexture(Resources.NO_MEMORY_CARD);
		else if(photoID == -1)
			mc.renderEngine.bindTexture(Resources.NO_PHOTOS);
		else if(photoID == 0)
			mc.renderEngine.bindTexture(Resources.LOADING);
		else mc.renderEngine.bindTexture(PhotosData.photos.get(photoID));
		if(!isEnabled) {
